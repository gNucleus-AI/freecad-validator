...
def derived_candidates(
    ...
    The match walks every spec key and classifies it via token presence;
    if a canonical name has a derived value, the spec key maps to it.
    Returns empty if the spec isn't a spline spec (no `spline` token
    anywhere) or if the base triple can't be extracted. ``bank`` is
    accepted for signature parity with the other categories' top-level
    ``derived_candidates`` functions, but spline derivation happens
    purely from the spec (CAD-side cross-checks live in the GearCategory).
    """
